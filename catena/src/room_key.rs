...
mod tests {
    ...
    fn serde_round_trip() {
        ...
        let back: RoomKey = serde_json::from_str(&json).unwrap();
        assert_eq!(key, back);
    }

    #[test]
    fn serde_rejects_invalid() {
        // No namespace separator — must fail validation on deserialize.
        let json = r#""nocolon""#;
        let err = serde_json::from_str::<RoomKey>(json);
        assert!(err.is_err());
    }
    ...
    mod prop {
        ...
        proptest! {
            ...
            fn arbitrary_string_never_panics(s in ".*") {
                // Construction must not panic — it either succeeds or returns an error.
                let _ = RoomKey::try_new(&s);
            }

            #[test]
            fn valid_keys_survive_round_trip(
                ns in "[a-z]{1,8}",
                local in "[a-z0-9/_-]{1,20}",
            ) {
                let raw = format!("{ns}:{local}");
                let key = RoomKey::try_new(&raw).unwrap();
                assert_eq!(key.as_str(), raw);
                assert_eq!(key.namespace(), ns.as_str());
            }

            #[test]
            fn empty_always_rejected(s in proptest::string::string_regex("").unwrap()) {
                assert_eq!(RoomKey::try_new(s), Err(RoomKeyError::Empty));
            }

            #[test]
            fn too_long_rejected(extra in "[a-z]{62,128}") {
                let raw = format!("ns:{extra}");
                assert!(raw.len() > MAX_LEN);
                assert!(matches!(RoomKey::try_new(raw), Err(RoomKeyError::TooLong(_))));
            }
            ...
            fn whitespace_rejected(
                ...
                let raw = format!("{pre}:{pre}{ws}{post}");
                assert!(matches!(
                    RoomKey::try_new(raw),
                    Err(RoomKeyError::Whitespace { .. })
                ));
            ...
            #[test]
            fn no_colon_rejected(s in "[a-z]{1,30}") {
                assert_eq!(RoomKey::try_new(s), Err(RoomKeyError::NoNamespace));
            }
        }
    }
}
